        ...
        <div className="mx-auto max-w-[1400px]">
          <article className="grid items-center gap-10 lg:grid-cols-2 lg:gap-20">
            ...
            <div className="flex flex-col items-start">
              <Link
                ...
                className="text-ink flex items-baseline gap-2 text-sm decoration-1 underline-offset-4 hover:underline"
              >
                <span aria-hidden="true">←</span>
                <span lang="zh" className="font-round-cjk">
                  {category.nameZh}
                </span>
                <span>{category.nameEn}</span>
              </Link>

              <p lang="zh" className="text-peach-text mt-10 text-2xl leading-none">
                {item.nameZh}
              </p>
              <h1 className="font-round mt-3 text-[clamp(2.25rem,4vw,3.75rem)] leading-[1.05] font-semibold">
                {item.nameEn}
              </h1>
              <p className="font-round mt-5 text-3xl font-semibold tabular-nums">
                {formatCad(item.priceCents)}
              </p>

              <DishTags item={item} className="mt-6 text-sm" />
              ...
              {item.ingredients ? (
                <section aria-labelledby="ingredients" className="mt-10 w-full">
                  <h2 id="ingredients" className="font-round text-xl font-semibold">
                    Ingredients
                  </h2>
              ...
              <div className="mt-10">
                <PaintButton href="/order" primary>
                  Order for pickup
                </PaintButton>
              </div>
          ...
          {more.length > 0 ? (
            <section aria-labelledby="more-dishes" className="mt-32">
              <h2
                id="more-dishes"
                className="font-round flex flex-wrap items-baseline gap-x-4 text-[clamp(1.75rem,3vw,2.75rem)] leading-tight font-semibold"
              >
                <span>More from</span>
                <span lang="zh" className="font-round-cjk text-peach-text">
                  {category.nameZh}
                </span>
              </h2>
